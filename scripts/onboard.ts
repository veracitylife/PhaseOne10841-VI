...
export function defaultAnswers(overrides: Partial<OnboardAnswers> = {}): OnboardAnswers {
  return {
    ...
    smtpFrom: process.env.SMTP_FROM ?? 'noreply@veracityintegrity.com',
    smtpSecure: process.env.SMTP_SECURE ?? 'false',
    useSmtp: Boolean(process.env.SMTP_HOST),
    databaseUrl:
      process.env.DATABASE_URL ?? 'postgres://phaseone:phaseone@localhost:5432/phaseone',
    sessionSecret: process.env.PHASEONE_SESSION_SECRET ?? generateSessionSecret(),
...
export function renderEnv(a: OnboardAnswers): string {
  const lines = [
    '# PhaseOne10841 v0.4 — generated by npm run onboard',
    '# Veracity Integrity LLC · https://VeracityIntegrity.com',
    ...
    '',
    '# --- SMTP (leave SMTP_HOST empty for console/dev OTP fallback) ---',
  ];
...
export async function runInteractive(root = ROOT): Promise<OnboardAnswers> {
  ...
  try {
    ...
    const adminEmails = await prompt(
      rl,
      'Admin email(s) for MFA (comma-separated)',
      base.adminEmails
    );
    ...
    if (useSmtp) {
      ...
      smtpFrom = await prompt(rl, 'SMTP from address', base.smtpFrom);
      smtpSecure = (await promptYesNo(rl, 'SMTP TLS (secure)?', false)) ? 'true' : 'false';
    } else {
      console.log('  → OTP will log to console + PHASEONE_OTP_FALLBACK_FILE (dev/CI friendly).');
    }
...
`);
  ...
  console.log(`  SMTP: ${answers.useSmtp && answers.smtpHost ? answers.smtpHost : 'console/fallback OTP'}`);
